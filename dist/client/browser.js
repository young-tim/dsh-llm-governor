...
export function GovernorModelSelect({ locked, available, sessionId, directory, load, selectModel, api, }) {
    ...
    return createElement('span', { className: 'dsh-governor-model-select' }, createElement('input', {
        ...
        : createElement('select', {
            ...
            : null, reasoning.efforts.map((effort) => createElement('option', { key: effort.id, value: effort.id }, effort.name))), saving ? createElement('span', { role: 'status' }, '保存中…') : null, error === null
        ? null
        : createElement('span', { className: 'dsh-governor-model-error', role: 'alert', title: error }, error));
}
...
function ModelsSettings({ api, canManage }) {
    ...
    return createElement('div', null, createElement(ErrorNotice, { error }), createElement('section', {
        ...
        : coveredTasks.length === 0
            ...
            : `Auto 部分就绪 · ${String(coveredTasks.length)}/7 类任务已覆盖`), createElement('p', null, 'Quality 是模型间的相对档位，不要求先做精确测评。每个模型先选一次 Lite 75、均衡 85 或 Pro 95，即可填满全部任务；之后只在“高级微调”里调整有证据的差异。'), missingTasks.length === 0
        ? null
        : createElement('small', null, `尚未覆盖：${missingTasks.join('、')}`)), createElement('div', { className: 'dsh-governor-table-wrap' }, createElement('table', null, createElement('thead', null, createElement('tr', null, ['模型', '启用', '倍率', '能力', '质量'].map((label) => createElement('th', { key: label }, label)))), createElement('tbody', null, rows.map((row) => createElement('tr', { key: row.routeId }, createElement('td', null, createElement('strong', null, row.model), createElement('small', null, row.provider)), createElement('td', null, createElement('input', {
        type: 'checkbox',
        checked: row.enabled,
        disabled: !canManage,
    ...
    })), createElement('td', null, createElement('div', { className: 'dsh-governor-quality-presets' }, createElement('span', null, `快速初始化 · 按名称建议 ${String(suggestedQualityPreset(row.model))}`), createElement('div', null, QUALITY_PRESETS.map((preset) => createElement('button', {
        key: preset.score,
        type: 'button',
        disabled: !canManage,
