 ...
 * @returns 兼容 trigger。
 */
export function deriveTrigger(causes) {
    for (const candidate of TRIGGER_PRIORITY) {
        if (causes.includes(candidate))
            return candidate;
    }
    return 'step';
}
 ...
 */
export function assertChangedFields(fields) {
    for (const field of fields) {
        if (!CHANGED_FIELDS.includes(field)) {
 ...
 */
export function assertEventSize(payload) {
