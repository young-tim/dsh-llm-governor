 ...
 * @returns 小写十六进制摘要。
 */
export function hmacInputHash(canonicalInput, hmacKey) {
 ...
 */
export function buildClassifierCacheKey(inputHash, classifierRoute, promptVersion, configRevision, tenantScope = 'default') {
 ...
 */
export class SQLiteClassifierCache {
    _repo;
    _hmacKey;
    _ttlMs;
    _classifierRoute;
    ...
    get(key) {
        ...
        if (Number.isFinite(createdAt) && Date.now() - createdAt > this._ttlMs)
            return undefined;
        return {
    ...
    buildKey(canonicalInput, configRevision) {
        const inputHash = hmacInputHash(canonicalInput, this._hmacKey);
        return buildClassifierCacheKey(inputHash, this._classifierRoute, this._promptVersion, configRevision, this._tenantScope);
    }
}
/**
