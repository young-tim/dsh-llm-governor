...
/** 1 Credit 对应的纳秒数。 */
export const NANOS_PER_CREDIT = 1000000000n;
/** Quality 取值下界（闭区间）。 */
const MIN_QUALITY = 0;
/** Quality 取值上界（闭区间）。 */
...
const DEFAULT_ROUTING_MODE = 'manual';
const DEFAULT_CREDIT_FIRST_MINIMUM_QUALITY = 0;
const DEFAULT_CREDIT_FIRST_ON_NO_MATCH = 'none';
...
const DEFAULT_FALLBACK_STRATEGY = 'quality_first';
...
const ROUTING_FIELDS = ['default', 'credit_first'];
const CREDIT_FIRST_FIELDS = ['minimum_quality', 'on_no_match'];
const AUTO_FIELDS = [
    'confidence_threshold',
    'quality_threshold',
    'llm_classifier',
];
const QUALITY_THRESHOLD_FIELDS = ['low', 'medium', 'high'];
const LLM_CLASSIFIER_FIELDS = ['enabled', 'provider', 'model', 'timeout_ms'];
...
const MODEL_FIELDS = ['enabled', 'multiplier', 'capabilities', 'quality'];
const USER_FIELDS = ['allow', 'monthly_credits'];
const STORAGE_FIELDS = ['enabled', 'path'];
const UI_FIELDS = ['enabled', 'port'];
...
/** 解析有限数字。 */
function parseNumber(value, path) {
...
function parseQualityValue(value, path) {
    const n = parseNumber(value, path);
...
/** 解析闭区间 [0, 1] 数字。 */
function parseUnitInterval(value, path) {
    const n = parseNumber(value, path);
    if (n < 0 || n > 1) {
...
function parseIdentityProvider(value, path) {
    const s = parseString(value, path);
    if (s !== 'local' && s !== 'header' && s !== 'jwt' && s !== 'custom') {
        throw new ConfigError(`provider must be local|header|jwt|custom, got "${s}"`, 'CONFIG_INVALID_PROVIDER', path);
    }
    return s;
}
/** 解析 RoutingMode 枚举。 */
function parseRoutingMode(value, path) {
    const s = parseString(value, path);
    if (s !== 'manual' && s !== 'quality_first' && s !== 'credit_first' && s !== 'auto') {
        throw new ConfigError(`routing mode must be manual|quality_first|credit_first|auto, got "${s}"`, 'CONFIG_INVALID_ROUTING_MODE', path);
    }
    return s;
}
/** 解析 FallbackStrategy 枚举。 */
function parseFallbackStrategy(value, path) {
    const s = parseString(value, path);
    if (s !== 'quality_first' && s !== 'credit_first' && s !== 'auto') {
        throw new ConfigError(`strategy must be quality_first|credit_first|auto, got "${s}"`, 'CONFIG_INVALID_FALLBACK_STRATEGY', path);
    }
    return s;
}
/** 解析 CreditFirstOnNoMatch 枚举。 */
function parseOnNoMatch(value, path) {
    const s = parseString(value, path);
    if (s !== 'quality_first' && s !== 'none') {
        throw new ConfigError(`on_no_match must be quality_first|none, got "${s}"`, 'CONFIG_INVALID_ON_NO_MATCH', path);
    }
    return s;
}
/** 校验 route_id 格式为 provider:model（非空 provider、单个冒号、非空 model）。 */
function parseRouteId(key, path) {
    const colonIdx = key.indexOf(':');
    if (colonIdx <= 0 || colonIdx === key.length - 1) {
        throw new ConfigError(`route id must be "provider:model" format`, 'CONFIG_INVALID_ROUTE_ID', `${path}.${key}`);
    }
}
/** 判断字符串是否为合法 TaskType。 */
function isValidTaskType(key) {
    return TASK_TYPE_SET.has(key);
}
/** 解析 Quality 映射，键必须是合法 TaskType，值必须在 [0, 100]。 */
function parseQuality(value, path) {
    if (!isObject(value)) {
    ...
    for (const [key, v] of Object.entries(value)) {
        if (!isValidTaskType(key)) {
            throw new ConfigError(`unknown task type "${key}"`, 'CONFIG_UNKNOWN_TASK_TYPE', `${path}.${key}`);
        }
...
function parseSchemaVersion(value) {
    ...
    if (typeof value !== 'number' || !Number.isInteger(value)) {
        throw new ConfigError(`schema_version must be integer ${SCHEMA_VERSION}`, 'CONFIG_SCHEMA_VERSION', 'schema_version');
    }
    if (value !== SCHEMA_VERSION) {
        throw new ConfigError(`schema_version must be ${SCHEMA_VERSION}, got ${value}`, 'CONFIG_SCHEMA_VERSION', 'schema_version');
    }
    return value;
}
// ===== Section 解析器 =====
...
function parseIdentity(value) {
    ...
    if (provider === 'header') {
        if (value['header_name'] === undefined) {
            throw new ConfigError('header_name is required when provider=header', 'CONFIG_MISSING_FIELD', 'identity.header_name');
        }
        // 强制声明可信代理来源（信任边界不可缺省）
        if (value['trusted_proxy'] === undefined) {
            throw new ConfigError('trusted_proxy is required when provider=header (trust boundary must be explicit)', 'CONFIG_MISSING_FIELD', 'identity.trusted_proxy');
        }
    }
    if (provider === 'jwt') {
        if (value['jwt_issuer'] === undefined) {
            throw new ConfigError('jwt_issuer is required when provider=jwt', 'CONFIG_MISSING_FIELD', 'identity.jwt_issuer');
        }
        if (value['jwt_audience'] === undefined) {
            throw new ConfigError('jwt_audience is required when provider=jwt', 'CONFIG_MISSING_FIELD', 'identity.jwt_audience');
        }
        if (value['jwt_algorithms'] === undefined) {
            throw new ConfigError('jwt_algorithms is required when provider=jwt', 'CONFIG_MISSING_FIELD', 'identity.jwt_algorithms');
        }
        // 签名密钥必须显式提供（禁止无密钥的"只 decode"部署）
        if (value['jwt_key'] === undefined && value['jwt_key_file'] === undefined) {
            throw new ConfigError('jwt_key or jwt_key_file is required when provider=jwt', 'CONFIG_MISSING_FIELD', 'identity.jwt_key');
        }
        if (value['jwt_key'] !== undefined && value['jwt_key_file'] !== undefined) {
            throw new ConfigError('jwt_key and jwt_key_file are mutually exclusive', 'CONFIG_CONFLICT', 'identity.jwt_key');
        }
    }
    // 解析可选字段；provider=local 时 local_user_id 默认 "local"
    const localUserId = value['local_user_id'] !== undefined
        ? parseNonEmptyString(value['local_user_id'], 'identity.local_user_id')
        : provider === 'local'
            ? DEFAULT_LOCAL_USER_ID
            : undefined;
    const headerName = value['header_name'] !== undefined
        ? parseNonEmptyString(value['header_name'], 'identity.header_name')
        : undefined;
    const trustedProxy = value['trusted_proxy'] !== undefined
        ? parseNonEmptyString(value['trusted_proxy'], 'identity.trusted_proxy')
        : undefined;
    const proxyHeaderName = value['proxy_header_name'] !== undefined
        ? parseNonEmptyString(value['proxy_header_name'], 'identity.proxy_header_name')
        : undefined;
    const displayNameHeader = value['display_name_header'] !== undefined
        ? parseNonEmptyString(value['display_name_header'], 'identity.display_name_header')
        : undefined;
    const emailHeader = value['email_header'] !== undefined
        ? parseNonEmptyString(value['email_header'], 'identity.email_header')
        : undefined;
    const jwtIssuer = value['jwt_issuer'] !== undefined
        ? parseNonEmptyString(value['jwt_issuer'], 'identity.jwt_issuer')
        : undefined;
    const jwtAudience = value['jwt_audience'] !== undefined
        ? parseNonEmptyString(value['jwt_audience'], 'identity.jwt_audience')
        : undefined;
    const jwtAlgorithms = value['jwt_algorithms'] !== undefined
        ? parseStringArray(value['jwt_algorithms'], 'identity.jwt_algorithms')
        : undefined;
    const jwtKey = value['jwt_key'] !== undefined ? parseString(value['jwt_key'], 'identity.jwt_key') : undefined;
    const jwtKeyFile = value['jwt_key_file'] !== undefined
        ? parseNonEmptyString(value['jwt_key_file'], 'identity.jwt_key_file')
        : undefined;
    const jwtSubjectClaim = value['jwt_subject_claim'] !== undefined
        ? parseNonEmptyString(value['jwt_subject_claim'], 'identity.jwt_subject_claim')
        : undefined;
    const jwtHeaderName = value['jwt_header_name'] !== undefined
        ? parseNonEmptyString(value['jwt_header_name'], 'identity.jwt_header_name')
        : undefined;
    const jwtScheme = value['jwt_scheme'] !== undefined
        ? parseString(value['jwt_scheme'], 'identity.jwt_scheme')
        : undefined;
    const jwtClockToleranceMs = value['jwt_clock_tolerance_ms'] !== undefined
    ...
    if (jwtAlgorithms !== undefined && jwtAlgorithms.length === 0) {
        throw new ConfigError('jwt_algorithms must not be empty', 'CONFIG_EMPTY_ARRAY', 'identity.jwt_algorithms');
    }
    return {
        provider,
...
function parseCredits(value) {
    ...
    const tokensPerCredit = value['tokens_per_credit'] !== undefined
    ...
    const timezone = value['timezone'] !== undefined
        ...
        : DEFAULT_TIMEZONE;
    const defaultMonthlyCredits = value['default_monthly_credits'] !== undefined
        ? creditsToNanos(parseNonNegativeNumber(value['default_monthly_credits'], 'credits.default_monthly_credits'))
        : creditsToNanos(DEFAULT_MONTHLY_CREDITS);
    return { tokensPerCredit, timezone, defaultMonthlyCredits };
}
/** 解析 credit_first 子块。 */
function parseCreditFirst(value) {
    if (value === undefined || value === null) {
        return {
...
function parseRouting(value) {
    ...
    const defaultMode = value['default'] !== undefined
...
function parseLlmClassifier(value) {
    ...
    if (enabled && (provider.length === 0 || model.length === 0)) {
        throw new ConfigError('llm_classifier.provider and llm_classifier.model are required when enabled', 'CONFIG_MISSING_FIELD', 'auto.llm_classifier');
    }
...
function parseAuto(value) {
    ...
    const confidenceThreshold = value['confidence_threshold'] !== undefined
...
function parseFallback(value) {
    ...
    const afterPartialOutput = value['after_partial_output'] !== undefined
...
/** 解析单个模型条目。 */
function parseModelEntry(value, path) {
    if (!isObject(value)) {
    ...
    const multiplier = value['multiplier'] !== undefined
    ...
    const capabilities = value['capabilities'] !== undefined
        ...
        : [];
    const quality = value['quality'] !== undefined ? parseQuality(value['quality'], `${path}.quality`) : {};
    return {
        enabled,
...
/** 解析单个用户条目。 */
function parseUserEntry(value, path) {
    if (!isObject(value)) {
        throw new ConfigError('expected object', 'CONFIG_TYPE_ERROR', path);
    }
    rejectUnknownKeys(value, USER_FIELDS, path);
    const allow = value['allow'] !== undefined ? parseStringArray(value['allow'], `${path}.allow`) : [];
    const monthlyCredits = value['monthly_credits'] !== undefined
...
function parseStorage(value) {
    ...
    const enabled = value['enabled'] !== undefined ? parseBoolean(value['enabled'], 'storage.enabled') : true;
    const path = value['path'] !== undefined ? parseNonEmptyString(value['path'], 'storage.path') : undefined;
...
function parseUi(value) {
    ...
    const enabled = value['enabled'] !== undefined ? parseBoolean(value['enabled'], 'ui.enabled') : true;
    const port = value['port'] !== undefined
        ? (() => {
            const p = parsePositiveInteger(value['port'], 'ui.port');
            if (p > 65535) {
                throw new ConfigError(`expected port in [1, 65535], got ${p}`, 'CONFIG_RANGE_ERROR', 'ui.port');
            }
            return p;
        })()
        : 0;
 ...
 */
export function multiplierToPpm(m) {
    if (!Number.isFinite(m) || m < 0) {
        throw new ConfigError('multiplier must be finite and non-negative', 'CONFIG_RANGE_ERROR', 'multiplier');
    }
 ...
 */
export function creditsToNanos(c) {
    if (!Number.isFinite(c) || c < 0) {
        throw new ConfigError('credits must be finite and non-negative', 'CONFIG_RANGE_ERROR', 'credits');
    }
 ...
 * 返回新的不可变配置对象，原对象不变。
 */
export function bumpRevision(config) {
 ...
 * 返回新的不可变配置对象。
 */
export function withRevision(config, revision) {
