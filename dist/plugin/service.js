...
export class GovernorService extends Service {
    ...
    _effectiveStrategy(mode, state, manualFallback) {
        if (mode === 'manual') {
            if (!manualFallback)
                return 'manual';
            if (this._fallbackStrategy === 'auto') {
    ...
    async selectModel(sessionId, turn, step, defaultConfig) {
        ...
        let attemptedMode = this._defaultRouting;
        let attemptedStrategy = this._effectiveStrategy(attemptedMode, state, false);
        try {
            ...
            const manualFallback = mode === 'manual' &&
    ...
    async updateModel(routeId, patch, options) {
        ...
        const newCapabilities = patch.capabilities === undefined
        ...
        this._modelDirectory = this._modelDirectory.map((s) => s.routeId === routeId
            ? {
                ...s,
                enabled: newEnabled,
                multiplierPpm: newMultiplierPpm,
