...
export const GovernorPlugin = {
    ...
    async apply(ctx, config) {
        ...
        ctx.on('agent/request', (async (payload, next) => {
            ...
            const { config } = service.selectModel(sessionId, payload.turn, payload.step, defaultConfig);
            return config;
        }), { global: true });
         ...
         */
        async function handleGovernorWeb(req, res, svc, provider) {
            const request = req;
            const response = res;
            ...
            await handle(request, response, GOVERNOR_WEB_PREFIX);
        }
    },
};
