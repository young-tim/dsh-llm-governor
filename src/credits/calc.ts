 ...
 */
export function computeCreditNanos(
  tokens: TokenCounts,
...
export function creditsToNanos(credits: number): bigint {
...
export function nanosToCredits(nanos: bigint): number {
  if (nanos < 0n) {
 ...
 */
export function validateNanosRange(nanos: bigint): void {
  if (nanos < MIN_SIGNED_64BIT || nanos > MAX_SIGNED_64BIT) {
    throw new RangeError(
      `NANOS_OUT_OF_RANGE: ${nanos} not in [${MIN_SIGNED_64BIT}, ${MAX_SIGNED_64BIT}]`,
    );
  }
}
