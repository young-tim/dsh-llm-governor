...
function zonedTimeToUtc(
  ...
  const get = (type: string): number => {
    ...
    if (!p) throw new Error(`DATETIME_PART_MISSING: ${type}`);
 ...
 */
export function monthWindow(timezone: string, at: Date = new Date()): { start: Date; end: Date } {
 ...
 */
export function checkQuota(usedNanos: bigint, limitNanos: bigint): QuotaStatus {
  if (usedNanos < 0n) throw new RangeError(`INVALID_USED_NANOS: ${usedNanos}`);
  if (limitNanos < 0n) throw new RangeError(`INVALID_LIMIT_NANOS: ${limitNanos}`);
