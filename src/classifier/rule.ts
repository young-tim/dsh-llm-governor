...
const SQL_KEYWORDS =
  /\b(?:SELECT|INSERT\s+INTO|UPDATE\s+\w+\s+SET|DELETE\s+FROM|CREATE\s+TABLE|ALTER\s+TABLE|DROP\s+TABLE|TRUNCATE\s+TABLE)\b/i;
/** Markdown 表格分隔行：| --- | 形式。 */
 ...
 */
function joinMessageText(input: ClassifyInput): string {
 ...
 * @returns 复杂度等级。
 */
export function complexityByLength(totalLength: number): Complexity {
 ...
 * @param input - 分类输入。
 */
export function classifyByRule(input: ClassifyInput): Classification | undefined {
