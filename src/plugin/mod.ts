...
import type { GovernorPrincipal } from '../security/governor-capabilities.js';
...
export const GovernorPlugin = {
  ...
  async apply(ctx: Context, config: GovernorPluginConfig): Promise<void> {
    ...
    const hostServices = ctx as unknown as { get?: (name: string) => unknown };
    const principalProvider = hostServices.get?.('governorPrincipal') as
      | { current(): GovernorPrincipal | undefined | Promise<GovernorPrincipal | undefined> }
      | undefined;
    const localPrincipal =
      resolved.identity.provider === 'local'
