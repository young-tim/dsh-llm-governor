...
export class UsageAggregator {
  ...
  private readonly _events = new Map<string, UsageEvent>();
  ...
  queryByModel(provider: string, model: string): UsageStats {
    const events = [...this._events.values()].filter(
   ...
   * 返回顺序为插入顺序（Map 迭代序）。
   */
  listEvents(filter?: { userId?: string; provider?: string }): UsageEvent[] {
   ...
   */
  private _computeStats(events: readonly UsageEvent[]): UsageStats {
    const attemptCount = events.length;
    if (attemptCount === 0) return emptyStats();
    ...
    for (const e of events) {
      ...
      if (e.fallbackIndex > 0) fallbackCount++;
