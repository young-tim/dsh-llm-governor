 ...
 */
export function generateCompatToken(): string {
...
export function createGovernorRequestHandler(
  ...
  const actors = new Map<string, GovernorActor>();
    ...
    if (path.startsWith('/api/')) {
      ...
      if (path === '/api/models' && method === 'GET') {
        if (!hasCapability(actor, 'governor.read')) {
          sendError(
            res,
            actor === undefined ? 401 : 403,
            actor === undefined ? 'UNAUTHORIZED' : 'FORBIDDEN',
          );
          return;
        }
      ...
      if (path === '/api/audit' && method === 'GET') {
        ...
        const limit = parseInt(url.searchParams.get('limit') ?? '50', 10);
        const entries = await governor.listAuditEntries(
          Math.min(Number.isFinite(limit) && limit > 0 ? limit : 50, 200),
        );
